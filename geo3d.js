 ...
 *
 * Three nested polyhedra rendered with Canvas 2D + manual 3D projection:
 *   - Outer: subdivided icosahedron (geodesic sphere, 42 vertices, 120 edges)
 *   - Middle: icosahedron (12 vertices, 30 edges)
 *   - Inner: octahedron (6 vertices, 12 edges)
 *
 * Features:
 ...
 *
 * Configuration: edit the `layerDefs` array below.
 *
...
(function(){
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     CONFIG — tweak colors, speeds, sizes here
     ═══════════════════════════════════════════════════════════════════════ */
  var CONFIG = {
    background: null,         // null = transparent, or 'rgba(...)' / '#hex'
    cameraZ: 3.5,             // camera distance (higher = smaller geometry)
    fovFactor: 0.9,           // field-of-view multiplier
    mouseSmooth: 0.035,       // mouse lerp speed (0–1, lower = smoother)
    breathe: 0.04,            // outer layer scale pulsation amplitude
    breatheSpeed: 1.5,        // pulsation speed
    subdivisions: 1,          // outer sphere subdivisions (1 = 42v, 2 = 162v)
  };

  /* Per-layer config:
     sc    = scale
     spd   = rotation speed [Y, X, Z]
     mInf  = mouse influence (0–1)
     lw    = line width (CSS px)
     la    = line alpha
     col   = RGB string for color
     pa    = point (vertex) glow alpha
     pr    = point glow radius (CSS px)
     dots  = draw vertex glow sprites? */
  var layerDefs = [
    { sc:1.5, spd:[.5,.35,.2],   mInf:.6, lw:0.6, la:.1,  col:'108,99,255',  pa:.25, pr:1.5, dots:false },
    { sc:.8,  spd:[-.8,-.55,-.35],mInf:.7, lw:1.2, la:.25, col:'0,212,170',   pa:.5,  pr:2.5, dots:true  },
    { sc:.35, spd:[1.3,.9,.7],   mInf:.9, lw:1.8, la:.45, col:'255,107,107',  pa:.7,  pr:4,   dots:true  },
  ];
  ...
  // Build subdivided icosahedron
  var subdiv = { v: icoV, f: icoF };
  for (var si = 0; si < CONFIG.subdivisions; si++) {
    subdiv = subdivide(subdiv.v, subdiv.f);
  }

  /* ═══════════════════════════════════════════════════════════════════════
     PREPARE GPU-FRIENDLY DATA STRUCTURES
     ═══════════════════════════════════════════════════════════════════════ */
  var geoSets = [
    { v: subdiv.v, e: extractEdges(subdiv.f) },
    { v: icoV,     e: extractEdges(icoF)     },
    { v: octV,     e: extractEdges(octF)     },
  ];

  var layers = [];
  for (var li = 0; li < 3; li++) {
    var def = layerDefs[li], geo = geoSets[li];
    ...
    layers.push({
      ...
      proj: new Float64Array(nv * 2),  // pre-allocated projection buffer
    });
  }
  ...
  var DPR = 1;
  function resize() {
    DPR = 1; // keep at 1 for perf; change to window.devicePixelRatio for retina
